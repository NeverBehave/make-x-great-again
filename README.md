...
<p align="center">
  <b>让 X 重新值得刷。</b><br>
  一个被动旁路扩展 · AI 替你拦垃圾 / 识水军 / 还能用回算法之前的 X
</p>
...
- 评论区被广告号和色情引流号淹没，正常的讨论沉底
- 想关注一个新 KOL，分不清是真号还是营销账号
- 算法决定你看到谁，而不是你决定
- 想找一个人聊过什么、最热的几条是什么 —— 只能手动往下翻

**Make X Great Again（MXGA）** 是一个**被动的浏览器扩展**：你只管正常刷 X，它在后台帮你做下面 5 件事。
凡是会动你账号的行为（拉黑、屏蔽），全部都得你亲自点一下；扩展只是把决策成本降到最低。

完全开源（AGPL-3.0）、不收任何个人数据、所有判定记录公开可查。
